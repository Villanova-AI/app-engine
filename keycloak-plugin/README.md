# Villanova Keycloak Plugin
Keycloak Integration for Villanova Core - Gives SSO capabilities and also has User Management through Keycloak.

For more information and documentation visit:  https://dev.villanova.org, or https://forum.villanova.org. Or for the latest news or product information please visit the main website: https://www.villanova.com.
...
* Enables SSO capabilities to an Villanova Instance by using Keycloak.
* Moves User Management to Keycloak.

### What this plugin does not
This plugin doesn't come with Role and Group management, because Villanova Core roles/groups model isn't compatible with Keycloak. That means that even with the same users across multiple Villanova Instances, the role and group mappings have to be configured on each instance.
...
>- `keycloak.realm`: The keycloak realm. See https://www.keycloak.org/docs/3.2/server_admin/topics/overview/concepts.html . (The default is `villanova`)
>- `keycloak.client.id`: The keycloak confidential client id. (The default is `villanova-app`)
>- `keycloak.client.secret`: The secret from the keycloak client. (The default is `<blank>`)
>- `keycloak.public.client.id`: The second keycloak client, this one must be public. (The default is `villanova-web`)
...
First add the `villanova-keycloak-auth` dependency to your pom.xml

```xml
<dependency>
    <groupId>org.villanova.villanova</groupId>
    <artifactId>villanova-keycloak-auth</artifactId>
...
keycloak.realm=${KEYCLOAK_REALM:villanova-development}
keycloak.client.id=${KEYCLOAK_CLIENT_ID:villanova-core}
keycloak.client.secret=${KEYCLOAK_CLIENT_SECRET:930837f0-95b2-4eeb-b303-82a56cac76e6}
keycloak.public.client.id=${KEYCLOAK_PUBLIC_CLIENT_ID:villanova-web}
...
## Keycloak Setup
In order to setup keycloak to work with villanova instance, please refer to the documentation here https://github.com/villanova/villanova-keycloak-plugin/wiki/Setup-Keycloak

## Keycloak Standard Flow
To enable the standard flow to keep sessions between Villanova instances, please refer to the documentation here
https://github.com/villanova/villanova-keycloak-plugin/wiki/Enable-Standard-Flow-for-Keycloak-Login
